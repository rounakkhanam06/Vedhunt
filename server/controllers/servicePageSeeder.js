...
// Hardcoded data mapped directly from client/src/constants/serviceDetailsData.js
// We are preserving 100% of the data and design.
...
const seedServicePages = async () => {
  try {
    const existingCount = await ServicePage.countDocuments();
    if (existingCount === 0) {
      logger.info('Seeding Service Pages...');
      
      const docsToInsert = Object.entries(SERVICE_DETAILS_DATA).map(([slug, data]) => ({
        slug,
        ...data
      }));

      await ServicePage.insertMany(docsToInsert);
      logger.info('Service Pages seeded successfully');
    } else {
      // Optional: Update existing to ensure structure matches
      // but if count > 0, we can skip for now to not overwrite manual changes
      logger.info('Service Pages already exist, skipping seed');
    }
  } catch (error) {
    logger.error('Error seeding Service Pages:', error);
  }
};
