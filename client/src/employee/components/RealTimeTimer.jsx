...
const RealTimeTimer = ({ activeTimer, tasks = [], onTimerStart, onTimerStop }) => {
  ...
  const handleStart = async (e) => {
    e.preventDefault();
    try {
      const res = await employeeApi.post('/employee-portal/ess/timer/start', { project, task, activityType });
      if (res.data.success) {
  ...
  const handleStop = async (e) => {
    e.preventDefault();
    try {
      const res = await employeeApi.post('/employee-portal/ess/timer/stop', {
        remarks,
        isProductive,
        isBillable,
        markTaskCompleted
