...
// notification" for a follow-up due right now, plus breaches).
const PRIORITY_TYPES = ['followup_due_priority', 'followup_breach'];
    ...
    <div className="relative" ref={wrapperRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 max-h-[calc(100vh-80px)] flex flex-col overflow-hidden bg-app-card border border-app-border rounded-xl shadow-lg z-50">
          <div className="px-4 py-3 border-b border-app-border shrink-0 flex items-center justify-between">
            <span className="text-sm font-bold text-app-text">Notifications</span>
            {pushAvailable && (
              <button
                onClick={() => enablePush(employeeApi, '/employee-portal/ess/notifications/register-device')}
            ...
            notifications.map((n) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 border-b border-app-border last:border-b-0 hover:bg-app-border/20 transition-colors ${isPriority ? 'border-l-2 border-l-red-500' : ''} ${!n.read ? 'bg-primary/5' : ''}`}
                >
                  <p className={`text-sm font-medium ${isPriority ? 'text-red-400' : 'text-app-text'}`}>{n.title}</p>
