...
const SubscriberManager = () => {
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-app-card border border-app-border rounded-xl p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-app-text-muted text-sm mb-1">Total Subscribers</p>
              <h3 className="text-2xl font-bold text-app-text">{stats.total}</h3>
            </div>
          </div>
        </div>
        <div className="bg-app-card border border-app-border rounded-xl p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-app-text-muted text-sm mb-1">Active</p>
              <h3 className="text-2xl font-bold text-app-text">{stats.active}</h3>
            </div>
          </div>
        </div>
        <div className="bg-app-card border border-app-border rounded-xl p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-app-text-muted text-sm mb-1">Unsubscribed</p>
              <h3 className="text-2xl font-bold text-app-text">{stats.inactive}</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Filters & Search */}
      <div className="bg-app-card border border-app-border rounded-xl p-4 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-96">
          <input
            ...
            className="w-full bg-form-input-bg border border-app-border rounded-lg py-2 pl-10 pr-4 text-app-text focus:outline-none focus:border-primary transition-colors"
          />
        ...
        <select
          ...
          className="bg-form-input-bg border border-app-border text-app-text rounded-lg py-2 px-4 focus:outline-none focus:border-primary cursor-pointer"
        >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-app-border">
              {loading ? (
                <tr>
                  <td colSpan="4" className="py-8 text-center text-app-text-muted">Loading subscribers...</td>
                </tr>
              ) : subscribers.length === 0 ? (
                <tr>
                  <td colSpan="5" className="py-8 text-center text-app-text-muted">No subscribers found.</td>
                </tr>
              ) : (
                subscribers.map((sub) => (
                  <tr key={sub._id} className="hover:bg-surface-variant transition-colors">
                    <td className="py-4 px-6 font-medium text-app-text">{sub.email}</td>
                    ...
                    <td className="py-4 px-6 text-right">
                      {sub.active && (
                        <button
                          ...
                          className="text-app-text-muted hover:text-secondary cursor-pointer"
          ...
          <div className="bg-app-bg px-6 py-4 flex items-center justify-between border-t border-app-border">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1 rounded bg-app-card text-app-text disabled:opacity-50 hover:bg-surface-variant transition-colors border border-app-border cursor-pointer disabled:cursor-not-allowed"
              >
                Previous
              </button>
              <button
      ...
      {showBroadcastModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-app-card border border-app-border rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleBroadcastSubmit} className="p-6 overflow-y-auto flex-1">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-app-text-muted mb-1">Email Subject</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full bg-form-input-bg border border-app-border rounded-lg px-4 py-3 text-app-text focus:outline-none focus:border-secondary transition-colors"
                  />
                </div>

                <div>
                  <label className="block text-sm font-medium text-app-text-muted mb-1 flex justify-between">
                    <span>Email Content</span>
                    <span className="text-xs text-app-text-muted">Unsubscribe link will be added automatically</span>
                  </label>
                  <textarea
                    required
                    rows={10}
                    ...
                    className="w-full bg-form-input-bg border border-app-border rounded-lg px-4 py-3 text-app-text focus:outline-none focus:border-secondary transition-colors text-sm"
                  />
                </div>
              </div>
              ...
              <div className="mt-8 flex gap-3 justify-end">
                <button
                  ...
                  className="px-6 py-2.5 rounded-lg border border-app-border text-app-text-muted hover:bg-surface-variant hover:text-app-text transition-colors disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed"
                >
                  Cancel
                </button>
                <button 
