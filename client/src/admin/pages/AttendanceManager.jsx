...
export default function AttendanceManager() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'Weekend': return <span className="px-2 py-1 bg-app-border/40 text-app-text-muted rounded text-xs font-bold uppercase tracking-wider">Weekend</span>;
      default: return <span className="px-2 py-1 bg-app-border/40 text-app-text-muted rounded text-xs font-bold uppercase tracking-wider">{status}</span>;
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex border-b border-app-border mb-8 overflow-x-auto">
      ...
      {activeTab === 'roster' && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-bold text-app-text">Daily Attendance Roster</h1>
              <p className="text-app-text-muted text-sm mt-1">View who is Present, Absent, Late, or on Half Day.</p>
            </div>
            <div className="w-full sm:w-auto">
              <input
                ...
                className="bg-app-card border border-app-border rounded-lg px-4 py-2 text-app-text outline-none w-full sm:w-auto"
              />
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        <div className="bg-app-card border border-app-border p-4 rounded-xl flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-app-text">{stats.present}</div>
            <div className="text-xs text-app-text-muted uppercase tracking-wider font-semibold">Present</div>
          </div>
        </div>
        <div className="bg-app-card border border-app-border p-4 rounded-xl flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-app-text">{stats.late}</div>
            <div className="text-xs text-app-text-muted uppercase tracking-wider font-semibold">Late</div>
          </div>
        </div>
        <div className="bg-app-card border border-app-border p-4 rounded-xl flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-app-text">{stats.halfDay}</div>
            <div className="text-xs text-app-text-muted uppercase tracking-wider font-semibold">Half Day</div>
          </div>
        </div>
        <div className="bg-app-card border border-app-border p-4 rounded-xl flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-app-text">{stats.absent}</div>
            <div className="text-xs text-app-text-muted uppercase tracking-wider font-semibold">Absent</div>
          </div>
        </div>
        <div className="bg-app-card border border-app-border p-4 rounded-xl flex items-center gap-4">
          ...
          <div>
            <div className="text-2xl font-bold text-app-text">{stats.onLeave}</div>
            <div className="text-xs text-app-text-muted uppercase tracking-wider font-semibold">On Leave</div>
          </div>
        </div>
        <div className="bg-app-card border border-app-border p-4 rounded-xl flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-red-500/10 text-red-500 flex items-center justify-center">
          ...
          <div>
            <div className="text-2xl font-bold text-app-text">{stats.forgotCheckout}</div>
            <div className="text-xs text-app-text-muted uppercase tracking-wider font-semibold">Forgot Checkout</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-app-card border border-app-border rounded-xl overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-app-text-muted">Loading roster...</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-app-text min-w-[800px]">
              <thead className="bg-app-bg text-xs uppercase text-app-text-muted border-b border-app-border">
                <tr>
              ...
              <tbody className="divide-y divide-app-border">
                {roster.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="px-6 py-8 text-center text-app-text-muted">No employees found</td>
                  </tr>
                ) : (
                  roster.map(r => (
                    <tr key={r.employeeId} className="hover:bg-surface-variant">
                      ...
                      <td className="px-6 py-4 font-mono font-semibold text-primary whitespace-nowrap">{r.workingHours || '-'}</td>
                    </tr>
                  ))
                )}
              </tbody>
