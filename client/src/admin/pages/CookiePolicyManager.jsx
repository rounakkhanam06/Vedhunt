...
const CookiePolicyManager = () => {
  ...
  const fetchPolicyData = async () => {
    try {
      setLoading(true);
      const res = await settingsService.getCookiePolicy();
      if (res.data) {
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await settingsService.updateCookiePolicy(policyData);
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto pb-24">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold text-white mb-2">Cookie Policy Manager</h2>
          <p className="text-gray-400">Manage the content of the Cookie Policy page.</p>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
